...
class App extends Component {
  ...
  componentDidMount() {
    ...
    const url = `http://192.168.45.174:10000/users/${userData.id}`;
    axios.get(url)
      .then(response => {
    ...
    const url1 = `http://192.168.45.174:30000/codereview/${this.props.match.params.id}`;
    axios.get(url1)
      .then(response => {
    ...
    const url2 = `http://192.168.45.174:40000/comment/${this.props.match.params.id}`;
    axios.get(url2)
      .then(response => {
  ...
  handleSubmitModal = () => {
    ...
    if (this.state.text.length) {
      const user = JSON.parse(sessionStorage.getItem('user'));
      const url = 'http://192.168.45.174:40000/comment';
  ...
  handleCompile = () => { //실행 버튼 클릭 했을 때
    ...
    const url = `http://192.168.45.174:30000/codereview/compilewindow/${getUrl[len - 1]}`
    axios.get(url)
      .then(response => {
        console.log(response);
        console.log(response.data)
