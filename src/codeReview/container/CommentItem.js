...
export default function CommentItem(props) {
  ...
  axios.get(`http://192.168.45.149:30103/comment/${props.comment_cmtId}`)
    .then(response => {
      if (response.data && response.data.exists) {
        // 존재한다면 reply insert
        axios.post('http://192.168.45.149:30103/comment/reply', {
  ...
  const handleRemove = () => {  //정말로 삭제
    ...
    const url = `http://192.168.45.149:30103/comment?cmtId=${comment_cmtId}`;
    axios.delete(url)
      .then(response => {
        console.log(response.data)
      }
